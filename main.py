...
def get_client_by_id(
    ...
    try:
        require_same_client(client_id, current_user)
...
def update_client_user(
    client_id: int,
    user_id: int,
    user_update: CompanyUserRequest,
    conn=Depends(get_db),
    _: TokenPayload = Depends(require_accountant),
):
    try:
...
def update_account_currency(
    account_id: int,
    acc_update: AccountCurrencyRequest,
    conn=Depends(get_db),
    _: TokenPayload = Depends(require_accountant),
):
    try:
...
def get_account_balances_from_account(
    account_id: int,
    conn=Depends(get_db),
    _: TokenPayload = Depends(get_current_user),
):
...
def get_all_financial_accounts_for_client(
    client_id: int,
    conn=Depends(get_db),
    current_user: TokenPayload = Depends(get_current_user),
):
...
def update_transaction_status(
    transaction_id: int,
    transaction_status_update: TransactionStatusRequest,
    conn=Depends(get_db),
    _: TokenPayload = Depends(get_current_user),
):
    try:
...
def update_category(
    category_id: int,
    category_update: CategoryRequest,
    conn=Depends(get_db),
    _: TokenPayload = Depends(get_current_user),
):
    try:
...
def update_cost_center(
    cost_center_id: int,
    cost_center_update: CostCenterRequest,
    conn=Depends(get_db),
    _: TokenPayload = Depends(get_current_user),
):
    try:
...
def get_account_transactions_from_account(
    account_id: int,
    conn=Depends(get_db),
    _: TokenPayload = Depends(get_current_user),
):
